...
/// Implements <see cref="IRequest{TResponse}"/> and expects a <see cref="AuthInfoModel"/> result on success.
/// </summary>
...
public record AuthWithUsernameRequest(string Username, string Password) : IRequest<AuthInfoModel>;

/// <summary>
/// Request to authenticate a user using a refresh token.
/// Implements <see cref="IRequest{UserAuthQueryModel}"/> and,
/// on success, returns a <see cref="AuthInfoModel"/> describing the authenticated user.
/// </summary>
...
public record AuthWithRefreshTokenRequest(string RefreshToken) : IRequest<AuthInfoModel>;
...
public record AuthWithExternalProviderRequest(string Provider, string OpenId) : IRequest<AuthInfoModel>;